...
void *server_side(void *socket_desc)
{
  ...
  serv_addr.sin_port = htons(5000);    //assigning 5000 as the port of the server
  ...
  while(1)
  {
	  ...
	  else if(recvBuff[i]=='g')
	  {
		for(i=0;i<in;i++)   // loop to chech if the number is present in the array or not
		{
		    if(arr[i]==no)
		    {
			strcpy(sendBuff,"THE NUMBER IS PRESENT IN 1ST SOCKET");
			flag=1;
			break;
		    }
		}
		if(flag==0)
		{
		    strcpy(sendBuff,"THE NUMBER IS NOT PRESENT IN 1ST SOCKET");
		}
		flag1=1;
		break;
	  }
